...
fn free_clone_wiring(vtable_name: &str) -> Result<FreeCloneWiring> {
    let free_symbol = super::shim::method_symbol(vtable_name, "free");
    let clone_symbol = super::shim::method_symbol(vtable_name, "clone");
    ...
    let declarations = format!(
        "static final _k$freeFast = $$ffi.Pointer.fromFunction<$$ffi.Void Function($$ffi.Uint64)>(_m$free);\n\
         ...
         \u{20}\u{20}try {{\n\
         \u{20}\u{20}\u{20}\u{20}_m$free(handle);\n\
         \u{20}\u{20}\u{20}\u{20}_f$signal_gate_ok(gate);\n\
         \u{20}\u{20}}} catch (_) {{\n\
         \u{20}\u{20}\u{20}\u{20}_f$signal_gate_error(gate);\n\
         \u{20}\u{20}}}\n\
         }}\n\
         static final _k$freeListener = _$$boltTrackListener($$ffi.NativeCallable<$$ffi.Void Function($$ffi.Uint64, $$ffi.Pointer<$$ffi.Void>)>.listener(_m$freeListenerEntry));\n\
         ...
         \u{20}\u{20}try {{\n\
         \u{20}\u{20}\u{20}\u{20}out.value = _m$clone(handle);\n\
         \u{20}\u{20}\u{20}\u{20}_f$signal_gate_ok(gate);\n\
         \u{20}\u{20}}} catch (_) {{\n\
         \u{20}\u{20}\u{20}\u{20}_f$signal_gate_error(gate);\n\
         \u{20}\u{20}}}\n\
         }}\n\
         static final _k$cloneListener = _$$boltTrackListener($$ffi.NativeCallable<$$ffi.Void Function($$ffi.Uint64, $$ffi.Pointer<$$ffi.Void>, $$ffi.Pointer<$$ffi.Uint64>)>.listener(_m$cloneListenerEntry));"
    );
...
fn shim_register_release_declaration(
    ...
    let register_symbol = super::shim::register_symbol(vtable_name);
    let release_symbol = super::shim::release_symbol(vtable_name);

    let native_signature = format!("$$ffi.Uint64 Function({})", argument_types.join(", "));
    let params = argument_types
        .iter()
        .enumerate()
