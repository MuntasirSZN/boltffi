...
/// Accessor generated by `templates/target/kotlin/class.kt` that rejects a
/// closed instance before handing out its native handle. Every handle read
/// crossing the FFI boundary goes through it, in receiver and argument position
/// alike, so sequential use-after-close raises instead of dereferencing freed
/// memory. The check is check-then-act: a `close()` racing between the check
/// and the native call can still release the handle first, so this is not a
