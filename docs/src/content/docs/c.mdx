...
That file names the archive explicitly and includes its dependencies. `pkg-config --static --libs mylib` also reports static dependencies, but its `-lmylib` flag alone does not force a compiler to choose the archive when a shared library is present.
...
Set `static: true` on `dependency(...)` to select the archive and its dependencies.
