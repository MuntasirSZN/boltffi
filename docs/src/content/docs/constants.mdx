---
title: "Constants"
description: "Export Rust constants as module-level values or associate them with records, enums, and classes across Swift, Kotlin, Java, C#, TypeScript, Python, and C."
---
...
Mark a standalone public constant with `#[export]`. Swift, Kotlin, TypeScript, Python, and C expose it at module scope. Java and C# place it on the generated module class because those languages do not have module-level values.
...
version = api_version`}
  c={`uint32_t version = DEMO_API_VERSION;
const char *service = DEMO_SERVICE_NAME;`}
/>
...
limit = Palette.MAX_COLORS`}
  c={`DemoColor black = demo_color_black();
uint8_t channels = DEMO_COLOR_CHANNEL_COUNT;
DemoMode mode = DEMO_MODE_DEFAULT;
uint8_t maximum = DEMO_PALETTE_MAX_COLORS;`}
/>
...
color = default_color
mode = default_mode`}
  c={`DemoColor color = demo_default_color();
DemoMode mode = DEMO_DEFAULT_MODE;`}
/>
