...
With the package name `mylib` and output directory `dist/c`, the package contains `include/mylib.h` and native libraries under `lib/`. Ship the header and libraries from the same build. No separate C runtime package is needed.
...
`pack c` builds for the machine running the command. Cross-compilation through `--target` or Cargo's `build.target` is rejected. Native C demo jobs cover Linux, macOS, and Windows; that does not imply every architecture or compiler combination has been tested. No CMake, Meson, or pkg-config package is generated.

Compile consumers as C11 or later, or include the header from C++. See [linking a C program](/docs/c#link-a-c-program) for compiler commands and runtime library paths. The [C demo](https://github.com/boltffi/boltffi/tree/main/examples/platforms/c) includes a CMake build and executable tests.
