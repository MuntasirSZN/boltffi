...
Enable `[targets.c]` in `boltffi.toml`, then build the header and native libraries for the current host:

```bash
boltffi pack c --experimental --release --deny-skipped
```

The output contains `include/<library>.h` and native libraries under `lib/`. The Rust library must declare both `cdylib` and `staticlib` crate types. C callers need a C11 compiler; the header can also be included from C++.

Linux gets `.so` and `.a` files; macOS gets `.dylib` and `.a`; Windows with MSVC gets `.dll`, `.dll.lib` for dynamic linking, and `.lib` for static linking. Windows GNU builds use `.dll`, `.dll.a`, and `.a`. The C demo is tested on Linux, macOS, and Windows.

`pack c` rejects cross-compilation settings, including Cargo's `--target` and `build.target`. `--deny-skipped` fails the build if C cannot represent an exported declaration. See the [C guide](/docs/c) for compiler commands, runtime library paths, ownership, callbacks, and current limitations.
