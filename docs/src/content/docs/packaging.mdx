...
package, a Python wheel, or a C header with native libraries. `generate` writes only the source bindings for a single language and is
...
Enable the [C target](/docs/configuration#c-configuration), then build the header and native libraries for the current host:

```bash
boltffi pack c --experimental --release --deny-skipped
```

The Rust library must declare both `cdylib` and `staticlib` crate types:
...
With the package name and Rust library target both named `mylib`, and output directory `dist/c`, the package contains:
...
Ship `include/` and `lib/` together from the same build. The CMake and pkg-config files use paths relative to the package, so the directory can be moved. No separate C runtime package is needed.
...
`pack c` builds for the machine running the command. Cross-compilation through `--target` or Cargo's `build.target` is rejected. Native C demo jobs cover Linux, macOS, and Windows; that does not imply every architecture or compiler combination has been tested. CMake and pkg-config metadata are generated automatically; Meson can consume the pkg-config file.

See [C configuration](/docs/configuration#c-configuration) for package names and versions. Native binaries keep the selected Cargo library target's name; the metadata points to those filenames, including the Windows import library.

Static link dependencies are collected from the same Rust build as the archive. `--no-build` reuses the header, libraries, and cached link metadata from a previous `pack c` run. Run without `--no-build` after changing the Rust build or if no cached metadata exists.

Compile consumers as C11 or later, or include the header from C++. See [linking a C program](/docs/c#link-a-c-program) for CMake, Make, Meson, and runtime library paths. The [C demo](https://github.com/boltffi/boltffi/tree/main/examples/platforms/c) includes a CMake build and executable tests.

### Header-only generation

```bash
boltffi generate c --experimental --deny-skipped
```

This writes `dist/c/boltffi.h` without packaging native libraries. `pack c` copies the public header to `dist/c/include/mylib.h` and builds the matching libraries.
