...
description: "Set up a BoltFFI Rust library, install prerequisites, configure packaging, and generate bindings for Apple, Android, Java, C#, WASM, Python, and C targets."
---
...
- For C, a C11 compiler such as GCC, Clang, or MSVC
...
Enable the experimental target in `boltffi.toml`:
...
Then build the header and libraries:

```sh
boltffi pack c --experimental --deny-skipped
```
...
The program prints `5`. The [C guide](/docs/c) also covers Windows linking, returned strings, collections, class handles, and cleanup.
