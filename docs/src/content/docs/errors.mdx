...
try:
    ...
    print(error)`}
  c={`DemoParseIntResult parsed = demo_parse_int(demo_string_view("42", 2));
if (parsed.ok) {
    printf("%d\\n", parsed.data.value);
} else {
    fwrite(parsed.data.error.ptr, 1, parsed.data.error.len, stderr);
}
...
try:
  ...
  c={`DemoParseConfigResult parsed = demo_parse_config(demo_string_view("invalid", 7));
if (!parsed.ok) {
    printf("line %u, column %u\\n", parsed.data.error.line, parsed.data.error.column);
    fwrite(parsed.data.error.message.ptr, 1, parsed.data.error.message.len, stderr);
}
demo_parse_config_result_free(&parsed);`}
/>
...
try:
    ...
    match error.error:
        ...
        case AuthError.ACCOUNT_LOCKED:
            print("Account is locked")`}
  c={`DemoLoginResult login = demo_login(
    demo_string_view("alice", 5), demo_string_view("wrong", 5));
if (!login.ok && login.data.error == DEMO_AUTH_ERROR_INVALID_CREDENTIALS) {
    fputs("invalid credentials\\n", stderr);
}
demo_login_result_free(&login);`}
/>
...
try:
  ...
  c={`DemoFetchUserResult fetched = demo_fetch_user(42);
if (!fetched.ok && fetched.data.error.tag == DEMO_API_ERROR_RATE_LIMITED) {
    printf("retry in %u seconds\\n", fetched.data.error.data.rate_limited.retry_after);
}
demo_fetch_user_result_free(&fetched);`}
/>
