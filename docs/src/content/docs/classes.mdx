...
counter.increment()
print(counter.get())  # 2`}
  c={`DemoCounter counter = demo_counter_new();
...
impl Database {
    pub fn new() -> Self {
        Database { path: ":memory:".into(), read_only: false }
    }
    
    pub fn open(path: &str) -> Self {
        Database { path: path.into(), read_only: false }
    }
...
db3 = Database.with_options("data.db", True)`}
  c={`DemoDatabase database = demo_database_open(demo_string_view("data.db", 7));
demo_database_free(&database);`}
/>
...
cfg = Config.production()`}
  c={`DemoConfig config = demo_config_production();
demo_config_free(&config);`}
/>
...
try:
    ...
    if error.error == DbError.NOT_FOUND:
        print("file not found")`}
  c={`DemoDatabaseOpenResult opened = demo_database_open(demo_string_view("data.db", 7));
if (!opened.ok && opened.data.error == DEMO_DB_ERROR_NOT_FOUND) {
    fputs("database not found\\n", stderr);
}
demo_database_open_result_free(&opened);`}
/>
...
try:
    ...
    print(error)`}
  c={`int64_t demo_account_balance(const DemoAccount *account);
void demo_account_deposit(const DemoAccount *account, int64_t amount);
DemoAccountWithdrawResult demo_account_withdraw(const DemoAccount *account, int64_t amount);`}
/>
...
t = Config.default_timeout()`}
  c={`uint32_t timeout = demo_config_default_timeout();
uint32_t maximum = demo_config_max_connections();`}
/>

### Async methods

The C target does not generate async methods yet. In the other targets, marking a method `async` produces an async method in the target language. BoltFFI has no built-in executor. You choose your Rust async runtime (Tokio, async-std, etc.), and the target language's async system coordinates with it automatically. See [Async](/docs/async) for more.
...
u = session.user()`}
  c={`void print_session_user(const DemoSession *session) {
    DemoUser user = demo_session_user(session);
    printf("%" PRIu64 "\\n", demo_user_id(&user));
    demo_user_free(&user);
}`}
/>
...
obj = MyClass()
print(obj.exported())  # 84`}
  c={`int32_t demo_my_class_exported(const DemoMyClass *instance);`}
/>
...
for thread in threads:
    thread.join()`}
  c={`DemoSafeCounter counter = demo_safe_counter_new();
demo_safe_counter_increment(&counter);
printf("%d\\n", demo_safe_counter_get(&counter));
demo_safe_counter_free(&counter);`}
/>
...
The Rust struct lives in Rust's heap. The target language holds a reference to it. In targets with automatic lifetime management, releasing the last foreign reference drops the Rust object. C uses explicit cleanup.

C callers release a class with its generated `<package>_<class>_free` function. A C struct assignment does not create another reference. In every target, keeping a class handle alive also keeps its Rust object alive.
