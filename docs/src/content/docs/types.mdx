...
try {
  ...
  python={`class ParseErrorException(Exception):
    pass
...
`std::time::Duration` represents a span of time. BoltFFI encodes it as seconds plus nanoseconds (12 bytes total). Swift uses `TimeInterval` (a `Double` of seconds), Kotlin and Java use `java.time.Duration`, C# uses a `long`, and TypeScript uses a `{ secs: bigint, nanos: number }` object.
...
const full = doubleDuration(half)`}
  python={`def sleep_duration() -> datetime.timedelta: ...
def double_duration(d: datetime.timedelta) -> datetime.timedelta: ...

half = sleep_duration()  # 0:00:00.500000
full = double_duration(half)`}
/>

### SystemTime

`std::time::SystemTime` represents a point in time. BoltFFI encodes it as seconds since Unix epoch plus nanoseconds (12 bytes). Swift uses `Date`, Kotlin and Java use `Instant`, C# uses a `long`, and TypeScript uses `Date`. Negative values represent times before 1970.
...
const unix = epoch()  // 1970-01-01T00:00:00.000Z`}
  python={`def now() -> datetime.datetime: ...
...
`uuid::Uuid` from the [uuid](https://crates.io/crates/uuid) crate maps to native UUID types in Swift, Kotlin, and Java. C# and TypeScript receive it as a string since those bindings do not use a generated UUID wrapper.
