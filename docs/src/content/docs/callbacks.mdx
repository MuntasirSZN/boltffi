...
C supports synchronous callback traits through a vtable and a typed handle. Its callback method signatures still use bridge types; see the [complete C example and ownership rules](/docs/c#callbacks). Inline closures and async callback traits are not generated for C.
