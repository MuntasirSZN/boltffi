...
This page covers free functions - standalone functions that aren't attached to a struct or class. For methods on objects, see [Classes](/docs/classes). For standalone values, see [Constants](/docs/constants).
...
double product = demo_multiply(2.0, 4.0);`}
/>
...
Primitive parameters pass directly with no overhead. C string inputs use borrowed views; returned strings own a separate allocation that the caller releases.
...
uintptr_t count = demo_char_count(demo_string_view(message.ptr, message.len));
printf("%zu\\n", (size_t)count);
...
d = distance(
  ...
  c={`DemoPoint start = {0.0, 0.0};
DemoPoint end = {3.0, 4.0};
double meters = demo_distance(start, end, DEMO_UNIT_METERS);`}
/>
...
avg = average([1.0, 2.0, 3.0])`}
  c={`int32_t integers[] = {1, 2, 3};
DemoI32Slice values = {integers, 3};
int32_t total = demo_sum(values);
double samples[] = {1.0, 2.0, 3.0};
DemoF64Slice readings = {samples, 3};
double average = demo_average(readings);`}
/>
...
casual = greet_user("John", None)`}
  c={`DemoOptionStringView title = {true, {"Dr.", 3}};
DemoString greeting = demo_greet_user(demo_string_view("Ali", 3), title);
demo_string_free(&greeting);
DemoOptionU64 no_callback = {false, 0};
demo_set_timeout(1000, no_callback);`}
/>
...
log_message(logger, "started")`}
  c={`DemoLogger logger = demo_logger_new(demo_string_view("app", 3));
demo_log_message(&logger, demo_string_view("started", 7));
demo_logger_free(&logger);`}
/>

### Callback traits

Functions can accept callback traits. A callback trait is a Rust trait marked with `#[export]` that the target language implements. Unlike closures (single anonymous function), callback traits can have multiple methods and carry state. C supports synchronous callback traits with the [limitations described in the C guide](/docs/c#callbacks); inline closures are not generated for C.

See [Callbacks](/docs/callbacks) for more.
...
pos = first_positive([-1, -2, 3])`}
  c={`int32_t input[] = {-1, 2, -3};
DemoI32Slice values = {input, 3};
DemoOptionI32 first = demo_first_positive(values);
if (first.has_value) {
    printf("%d\\n", first.value);
}`}
/>
...
} else if (port.data.error == DEMO_PARSE_ERROR_OUT_OF_RANGE) {
...
pos = filter_positive([-1, 2, -3, 4])`}
  c={`DemoI32Sequence values = demo_range(0, 10);
printf("%zu values\\n", (size_t)values.len);
demo_i32_sequence_free(&values);`}
/>

## Async functions

C does not generate async functions yet. The following examples apply to the other targets.
...
C does not generate inline closure parameters yet. Synchronous callback traits have a [C vtable API](/docs/c#callbacks).
