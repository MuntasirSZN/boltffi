...
`boltffi generate c --experimental` writes the intermediate header at
`{output}/boltffi.h`. `boltffi pack c --experimental` stages the public header
at `{output}/include/<library>.h` and native libraries under `{output}/lib`.
C is disabled by default; its default output directory is `dist/c`. The package name supplies prefixes such as `MylibPoint` and `mylib_distance`.

Pass `--deny-skipped` to make unsupported exports fail generation or packaging. To enable C in `generate all` or `pack all`, keep `enabled = true` and either pass `--experimental` or set the top-level `experimental = ["c"]` array. C packaging is host-only; it rejects Cargo target overrides. See [C](/docs/c) for linking and ownership rules.
