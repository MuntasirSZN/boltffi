---
title: "Configuration"
description: "Configure boltffi.toml package names, target outputs, and platform packaging behavior for generated Swift, Kotlin, Java, C#, TypeScript, Python, and C bindings."
---
...
<h2 id="c-configuration">C Configuration (experimental)</h2>

Enable the experimental C target and choose its output root:
...
C is disabled by default; its default output directory is `dist/c`. To include it in `generate all` or `pack all`, keep `enabled = true` and either pass `--experimental` or set `experimental = ["c"]` at the top of `boltffi.toml`, before any section. Saving the experimental opt-in does not enable `targets.c` by itself.

The [package name](#package-identity) supplies the C prefix. With `[package] name = "demo"`, a Rust `Point` becomes `DemoPoint`, `add` becomes `demo_add`, and `ANSWER` becomes `DEMO_ANSWER`. Methods include the type name, such as `demo_counter_get`. Names beginning with `boltffi_`, `Ffi`, or `___` belong to the underlying bridge; application code normally uses the package-prefixed API.

See [C packaging](/docs/packaging#c-experimental) for generation commands, output files, and host restrictions.
