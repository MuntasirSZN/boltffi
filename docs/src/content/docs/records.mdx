...
Classes are different. A class lives in Rust, and the target language holds a reference to it. See [Classes](/docs/classes) for when to use one over the other. Records may also define [associated constants](/docs/constants#associated-constants).
...
p = Point(50, 50)
inside = r.contains(p)`}
  c={`DemoRect rectangle = {0.0, 0.0, 10.0, 10.0};
DemoPoint point = {3.0, 4.0};
bool inside = demo_rect_contains(&rectangle, point);`}
/>
...
result = search_users("alice")`}
  c={`DemoSearchResult result = demo_search_users(demo_string_view("Ali", 3));
printf("%zu matches\\n", (size_t)result.matches.len);
demo_search_result_free(&result);`}
/>
...
summary = company_summary(co)`}
  c={`DemoCompanyView company = {
    {"Acme", 4},
    {{"Main St", 7}, {"Amsterdam", 9}, {"1000", 4}},
    20
};
DemoString summary = demo_company_summary(company);
fwrite(summary.ptr, 1, summary.len, stdout);
...
label = display_label(p)`}
  c={`DemoProfileView profile = {
    .username = {"ali", 3},
    .display_name = {.has_value = false},
    .bio = {.has_value = false},
    .follower_count = 0
};
DemoString label = demo_display_label(profile);
...
Use `#[boltffi::default(...)]` on a field to give it a default value. Targets with default constructor arguments make that parameter optional. C uses aggregate initialization, so C callers must supply the field values explicitly.
...
In targets with default constructor arguments, option fields default to `nil`/`null` when no explicit default is provided. In C, initialize `has_value` to `false` for an absent value.
...
label = status_label(Status.ACTIVE)`}
  c={`DemoStatus status = DEMO_STATUS_ACTIVE;
DemoString label = demo_status_label(status);
fwrite(label.ptr, 1, label.len, stdout);
demo_string_free(&label);`}
/>
...
desc = describe_state(state)`}
  c={`DemoLoadStateView state = {
    .tag = DEMO_LOAD_STATE_LOADING,
    .data.loading = {.progress = 0.5}
};
DemoString description = demo_describe_state(state);
fwrite(description.ptr, 1, description.len, stdout);
...
u = Point.try_unit(3.0, 4.0)`}
  c={`DemoPoint point = demo_point_new(3.0, 4.0);
DemoPoint origin = demo_point_origin();
DemoPointTryUnitResult unit = demo_point_try_unit(point.x, point.y);
if (unit.ok) {
    printf("%g, %g\\n", unit.data.value.x, unit.data.value.y);
}
demo_point_try_unit_result_free(&unit);`}
/>
...
d = p.distance()       # 5.0
sum_value = p.add(q)`}
  c={`DemoPoint point = {3.0, 4.0};
DemoPoint offset = {1.0, 2.0};
double distance = demo_point_distance(&point);
DemoPoint moved = demo_point_add(&point, offset);`}
/>
...
dims = Point.dimensions()  # 2`}
  c={`uint32_t dimensions = demo_point_dimensions();`}
/>
...
e = Direction.from_degrees(90)`}
  c={`DemoDirection north = DEMO_DIRECTION_NORTH;
DemoDirection south = demo_direction_opposite(&north);
DemoDirection east = demo_direction_from_degrees(90.0);`}
/>
