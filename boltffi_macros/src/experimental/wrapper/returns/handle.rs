...
impl<'a> Render<Native, ValueInput<'a, native::HandleCarrier>> for Value {
    type Output = ValueTokens;

    fn render(self, input: ValueInput<'a, native::HandleCarrier>) -> Result<Self::Output, Error> {
        ClassReturn::new(input).tokens::<Native>()
    }
}

impl<'a> Render<Wasm32, ValueInput<'a, wasm32::HandleCarrier>> for Value {
    type Output = ValueTokens;

    fn render(self, input: ValueInput<'a, wasm32::HandleCarrier>) -> Result<Self::Output, Error> {
