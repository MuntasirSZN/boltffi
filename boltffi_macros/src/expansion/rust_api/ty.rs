...
impl DecodeTarget {
    pub fn new(
        ...
        match (passing, receive) {
            ...
            (ParameterPassing::Ref, Receive::ByRef) => Ok(Self::borrowed(type_expr, false)?),
            (ParameterPassing::RefMut, Receive::ByMutRef) => Ok(Self::borrowed(type_expr, true)?),
    ...
    fn borrowed(type_expr: &TypeExpr, mutable: bool) -> Result<Self, Error> {
