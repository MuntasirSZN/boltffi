...
use boltffi_binding::{
    ...
    DirectValueType, EncodedFieldDecl, EnumDecl, FieldKey, IntegerRepr, Native, Wasm32,
};
...
pub struct Enumeration<'expansion, 'lowered, S: boltffi_binding::SurfaceLower> {
    pair: DeclarationPair<'lowered, EnumDef, EnumDecl<S>>,
    expansion: &'expansion Expansion<'lowered, S>,
}

#[derive(Clone, Copy)]
struct CStyle<'expansion, 'lowered, S: boltffi_binding::SurfaceLower> {
...
#[derive(Clone, Copy)]
struct Data<'expansion, 'lowered, S: boltffi_binding::SurfaceLower> {
    source: &'lowered EnumDef,
    binding: &'lowered DataEnumDecl<S>,
    expansion: &'expansion Expansion<'lowered, S>,
}

impl<'expansion, 'lowered, S: boltffi_binding::SurfaceLower> Enumeration<'expansion, 'lowered, S> {
    pub fn new(
        pair: DeclarationPair<'lowered, EnumDef, EnumDecl<S>>,
        expansion: &'expansion Expansion<'lowered, S>,
    ) -> Self {
...
impl<'expansion, 'lowered, S: boltffi_binding::SurfaceLower> CStyle<'expansion, 'lowered, S> {
...
impl<'expansion, 'lowered, S: boltffi_binding::SurfaceLower> Data<'expansion, 'lowered, S> {
    fn runtime(self) -> Result<TokenStream, Error> {
        let enumeration = names::SourceSpelling::new(&self.source.name)
            .ident("source enum name is not a Rust identifier")?;
        let variants = self.variants()?;
...
struct DataVariant<'expansion, 'lowered, S: boltffi_binding::SurfaceLower> {
    source: &'lowered VariantDef,
    binding: &'lowered DataVariantDecl,
    fields: Vec<DataField<'expansion, 'lowered, S>>,
}

struct DataField<'expansion, 'lowered, S: boltffi_binding::SurfaceLower> {
...
impl<'expansion, 'lowered, S: boltffi_binding::SurfaceLower> DataVariant<'expansion, 'lowered, S> {
...
impl<'expansion, 'lowered, S: boltffi_binding::SurfaceLower> DataField<'expansion, 'lowered, S> {
