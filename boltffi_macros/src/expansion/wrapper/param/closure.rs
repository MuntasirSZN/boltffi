...
#[derive(Clone, Copy)]
enum ForeignClosure<'lowered, S: boltffi_binding::SurfaceLower> {
    Parameter(&'lowered ClosureParameter<S, IntoRust>),
    Return(&'lowered ClosureReturn<S, IntoRust>),
}

impl<'lowered, S: boltffi_binding::SurfaceLower> ForeignClosure<'lowered, S> {
...
struct ClosureInvoke<'expansion, 'lowered, 'rust, S: boltffi_binding::SurfaceLower> {
...
impl<'expansion, 'lowered, 'rust, S: boltffi_binding::SurfaceLower>
    ClosureInvoke<'expansion, 'lowered, 'rust, S>
{
    fn new(
        callable: &'lowered ImportedCallable<S>,
        source: &'lowered FnSig,
...
impl<'expansion, 'lowered, 'rust> ClosureInvoke<'expansion, 'lowered, 'rust, Native> {
...
impl<'expansion, 'lowered, 'rust> ClosureInvoke<'expansion, 'lowered, 'rust, Wasm32> {
    fn return_tokens(&self) -> Result<ForeignClosureReturnTokens, Error> {
        ForeignClosureReturn::new(
            self.callable.returns().plan(),
            self.callable.error(),
...
struct InvokeParameterInput<'expansion, 'lowered, 'rust, S: boltffi_binding::SurfaceLower> {
    index: usize,
...
struct ForeignClosureReturn<'expansion, 'lowered, S: boltffi_binding::SurfaceLower> {
...
impl<'expansion, 'lowered> ForeignClosureReturn<'expansion, 'lowered, Native> {
    fn render(self) -> Result<ForeignClosureReturnTokens, Error> {
        ...
        match (self.plan, self.error) {
            (ReturnPlan::ScalarOptionViaReturnSlot { primitive, .. }, ErrorDecl::None(_)) => {
                rust_api::Return::new(self.source).scalar_option(*primitive)?;
                let rust_type = self.rust_type.as_ref().ok_or(Error::SourceSyntaxMismatch(
                    "closure optional scalar return requires source return type",
                ))?;
                let value = wrapper::returns::scalar_option::IncomingInput::new(
                    *primitive,
                    rust_type.clone(),
            ...
            (ReturnPlan::DirectVecViaReturnSlot { .. }, ErrorDecl::None(_)) => {
                let element = rust_api::Return::new(self.source).direct_vec_element_type()?;
                let value = wrapper::returns::direct_vec::IncomingInput::new(
                    element,
                    quote! { __boltffi_result_buf },
                )
                .native()?;
            ...
            (
                ...
                let result = self.rust_fallible_return()?;
                let ok = self.encoded_expression(
                    ok_codec,
                    &result.ok_type,
                    &result.ok_source,
            ...
            (
                ...
                let value = self.encoded_expression(
                    codec,
                    rust_type,
                    source_type,
...
impl<'expansion, 'lowered> ForeignClosureReturn<'expansion, 'lowered, Wasm32> {
    fn render(self) -> Result<ForeignClosureReturnTokens, Error> {
        if let Some(tokens) = self.direct_tokens()? {
            return Ok(tokens);
        }
        ...
        match (self.plan, self.error) {
            ...
            (
                ...
                let value = self.packed_expression(
                    codec,
                    rust_type,
                    source_type,
...
impl ForeignClosureReturnTokens {
    ...
    fn body(&self, call: TokenStream) -> TokenStream {
        match self {
            ...
            Self::NativeFallibleEncoded { ok, error } => {
                let setup = self.setup();
                quote! {
                    #(#setup)*
                    let __boltffi_error_buf = unsafe { #call };
...
impl ClosureBinding {
    fn new(
        ...
        let signature = ClosureSignature::from_source(source.signature())?;
...
impl ClosureSignature {
    fn from_source(source: &FnSig) -> Result<Self, Error> {
