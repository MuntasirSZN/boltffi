...
struct AsyncExports<'expansion, 'lowered, S: boltffi_binding::SurfaceLower> {
    symbol: &'lowered NativeSymbol,
    callable: &'lowered ExportedCallable<S>,
    source: rust_api::Callable<'lowered>,
    rust_call: export::RustCall,
...
impl<'expansion, 'lowered> AsyncExports<'expansion, 'lowered, Native> {
    fn new(
        symbol: &'lowered NativeSymbol,
        callable: &'lowered ExportedCallable<Native>,
        source: rust_api::Callable<'lowered>,
        rust_call: export::RustCall,
        receiver: export::ReceiverTokens,
        visibility: TokenStream,
    ...
    fn tokens(self, protocol: NativeProtocol) -> Result<TokenStream, Error> {
        ...
        let poll = protocol.poll(visibility, rust_return_type);
        let complete = protocol.complete(visibility, rust_return_type, self.complete);
        let panic_message = protocol.panic_message(visibility, rust_return_type);
        let cancel = protocol.cancel(visibility, rust_return_type);
        let free = protocol.free(visibility, rust_return_type);
...
impl<'expansion, 'lowered> AsyncExports<'expansion, 'lowered, Wasm32> {
    fn new(
        symbol: &'lowered NativeSymbol,
        callable: &'lowered ExportedCallable<Wasm32>,
        source: rust_api::Callable<'lowered>,
        rust_call: export::RustCall,
        receiver: export::ReceiverTokens,
        visibility: TokenStream,
    ...
    fn tokens(self, protocol: WasmProtocol) -> Result<TokenStream, Error> {
...
struct CapturedParameters<'lowered, S: boltffi_binding::SurfaceLower> {
    params: &'lowered [ParamDecl<S, IntoRust>],
}

impl<'lowered, S: boltffi_binding::SurfaceLower> CapturedParameters<'lowered, S> {
...
impl Complete {
    ...
    fn wasm32<'expansion, 'plan>(
        ...
        PlainComplete::wasm32(owner, callable, source, rust_return_type, expansion).map(Self::Plain)
    }
    ...
    fn tokens(
        ...
        match self {
            Self::Plain(complete) => complete.tokens(cfg, visibility, ident, rust_return_type),
            Self::Fallible(complete) => complete.tokens(cfg, visibility, ident),
        }
    }
}
    ...
    fn native<'expansion, 'plan>(
        ...
        match callable.returns().plan() {
            ...
            ReturnPlan::EncodedViaReturnSlot { codec, shape, .. } => {
                let encoded =
                    encoded::Input::new(codec, *shape, result.clone(), expansion).render()?;
                let empty = encoded::Empty::<Native>::new(*shape).render()?;
                Ok(Self {
                    items: Vec::new(),
                    ffi_parameters: Vec::new(),
            ...
            ReturnPlan::HandleViaReturnSlot {
                ...
                .render()?;
                let carrier = wrapper::handle::CarrierTokens::native(*carrier)?;
                let ty = handle.ty();
                Ok(Self {
                    items: Vec::new(),
                    ffi_parameters: Vec::new(),
            ...
            ReturnPlan::ScalarOptionViaReturnSlot {
                ...
                .native()?;
                let empty = scalar_option::Empty::new(*primitive).native()?;
                Ok(Self {
                    items: Vec::new(),
                    ffi_parameters: Vec::new(),
            ...
            ReturnPlan::DirectVecViaReturnSlot { .. } => {
                ...
                let empty = wrapper::returns::Tokens::empty_native();
                Ok(Self {
                    items: Vec::new(),
                    ffi_parameters: Vec::new(),
            ...
            ReturnPlan::ClosureViaOutPointer(closure) => {
                ...
                let writer = closure::WriteInput::returned(
                    closure,
                    source_closure,
                    result.clone(),
                    owner,
    ...
    fn wasm32<'expansion, 'plan>(
        ...
        match callable.returns().plan() {
            ...
            ReturnPlan::EncodedViaReturnSlot { codec, shape, .. } => {
                let encoded =
                    encoded::Input::new(codec, *shape, result.clone(), expansion).render()?;
                let empty = encoded::Empty::<Wasm32>::new(*shape).render()?;
                Ok(Self {
                    items: Vec::new(),
                    ffi_parameters: Vec::new(),
                    return_type: encoded.return_type().clone(),
            ...
            ReturnPlan::HandleViaReturnSlot {
                ...
                .render()?;
                let carrier = wrapper::handle::CarrierTokens::wasm32(*carrier)?;
                let ty = handle.ty();
                Ok(Self {
                    items: Vec::new(),
                    ffi_parameters: Vec::new(),
            ...
            ReturnPlan::ScalarOptionViaReturnSlot {
                ...
                .wasm32()?;
                let empty = scalar_option::Empty::new(*primitive).wasm32()?;
                Ok(Self {
                    items: Vec::new(),
                    ffi_parameters: Vec::new(),
                    return_type: optional.return_type().clone(),
            ...
            ReturnPlan::DirectVecViaReturnSlot { .. } => {
                ...
                let empty = wrapper::returns::Tokens::empty_wasm32();
                Ok(Self {
                    items: Vec::new(),
                    ffi_parameters: Vec::new(),
                    return_type: sequence.return_type().clone(),
...
impl FallibleComplete {
    fn native<'expansion, 'plan>(
        ...
        let encoded_error =
            encoded::Input::new(codec, *shape, error.clone(), expansion).render()?;
        let empty_error = encoded::Empty::<Native>::new(*shape).render()?;
        let success =
            fallible::SuccessInput::new(callable.returns(), source, owner, expansion).render()?;
    ...
    fn wasm32<'expansion, 'plan>(
        ...
        let encoded_error =
            encoded::Input::new(codec, *shape, error.clone(), expansion).render()?;
        let empty_error = encoded::Empty::<Wasm32>::new(*shape).render()?;
        let success =
            fallible::SuccessInput::new(callable.returns(), source, owner, expansion).render()?;
        Self::finish(error, encoded_error, empty_error, success, rust_return_type)
    }
    ...
    fn finish(
        error: syn::Ident,
        encoded_error: encoded::Tokens,
        empty_error: encoded::Tokens,
        success: fallible::SuccessTokens,
        rust_return_type: &Type,
    ) -> Result<Self, Error> {
    ...
    fn tokens(self, cfg: TokenStream, visibility: &TokenStream, ident: syn::Ident) -> TokenStream {
        let Self {
            ffi_parameters,
            return_type,
            body,
...
impl<'render> FutureSupport<'render> {
    ...
    fn panic_message(&self, cfg: TokenStream, ident: syn::Ident) -> TokenStream {
        let visibility = self.visibility;
        let rust_return_type = self.rust_return_type;
        quote! {
            #cfg
    ...
    fn cancel(&self, cfg: TokenStream, ident: syn::Ident) -> TokenStream {
        let visibility = self.visibility;
        let rust_return_type = self.rust_return_type;
        quote! {
            #cfg
    ...
    fn free(&self, cfg: TokenStream, ident: syn::Ident) -> TokenStream {
        let visibility = self.visibility;
        let rust_return_type = self.rust_return_type;
        quote! {
            #cfg
