...
impl<'expansion, 'lowered, S: SurfaceLower> Trait<'expansion, 'lowered, S> {
    pub fn new(
        pair: DeclarationPair<'lowered, TraitDef, CallbackDecl<S>>,
        expansion: &'expansion Expansion<'lowered, S>,
    ) -> Self {
...
struct MethodPair<'lowered, S: SurfaceLower, TargetName> {
    source: &'lowered MethodDef,
    binding: &'lowered ImportedMethodDecl<S, TargetName>,
    local: Option<&'lowered CallbackLocalMethodDecl<S>>,
}

struct CallbackMethods<'lowered, S: SurfaceLower, TargetName> {
    methods: Vec<MethodPair<'lowered, S, TargetName>>,
}

impl<'lowered, S: SurfaceLower, TargetName> CallbackMethods<'lowered, S, TargetName> {
    fn new(
        source: &'lowered [MethodDef],
        binding: &'lowered [ImportedMethodDecl<S, TargetName>],
        local: Option<&'lowered [CallbackLocalMethodDecl<S>]>,
...
struct MethodAbi<'expansion, 'lowered, S: SurfaceLower> {
    source: &'lowered MethodDef,
...
struct MethodParameter<'expansion, 'lowered, S: SurfaceLower> {
    ...
    fn foreign_direct_tokens(
        ...
        match (self.source.passing, ty) {
            ...
            (ParameterPassing::Value, DirectValueType::Record(_)) => {
                ...
                S::foreign_direct_record_parameter(rust_type, ident.as_ident().clone(), packed)
            }
    ...
    fn foreign_direct_vec_tokens(
        ...
        match element {
            DirectVectorElementType::Primitive(primitive) => {
                let element_type = wrapper::type_ref::primitive(primitive.primitive())?;
                Ok(ForeignMethodParameterTokens::new(
        ...
        match error {
            ErrorDecl::None(_) => match InfallibleMethodReturn::from_plan(plan)? {
                ...
                InfallibleMethodReturn::ScalarOption { primitive } => {
                    ...
                    let optional = S::outgoing_scalar_option(
                ...
                InfallibleMethodReturn::DirectVec => {
                    ...
                    let result = wrapper::names::Locals::new(Span::call_site()).result();
                    let sequence = S::outgoing_direct_vector(
    ...
    fn native_async_foreign_body(&self, call: TokenStream) -> Result<TokenStream, Error> {
        ...
        match InfallibleMethodReturn::new(self.plan, self.error)? {
            ...
            InfallibleMethodReturn::ScalarOption { primitive } => {
                ...
                let rust_type = self.direct_source_type()?;
                let value = S::incoming_scalar_option(
            ...
            InfallibleMethodReturn::DirectVec => {
                ...
                let value =
                    S::incoming_direct_vector(wrapper::returns::direct_vec::IncomingInput::new(
                        element,
                        quote! { __boltffi_result },
                    ))?;
                Ok(self.native_async_bytes_body(call, value))
    ...
    fn foreign_body(&self, call: TokenStream) -> Result<TokenStream, Error> {
        ...
        match InfallibleMethodReturn::new(self.plan, self.error)? {
            ...
            InfallibleMethodReturn::ScalarOption { primitive } => {
                self.source.scalar_option(primitive)?;
                let rust_type = self.direct_source_type()?;
                S::incoming_scalar_option(wrapper::returns::scalar_option::IncomingInput::new(
                    primitive,
                    rust_type,
                    quote! { unsafe { #call } },
                ))
    ...
    fn local_body(&self, owner: Ident, call: TokenStream) -> Result<LocalMethodBody, Error> {
        ...
        match InfallibleMethodReturn::new(local_return.plan, local_return.error)? {
            ...
            InfallibleMethodReturn::Closure { closure } => {
                ...
                let writer =
                    S::write_closure_return(wrapper::returns::closure::WriteInput::returned(
                        closure,
                        self.source.closure(closure.presence())?,
                        result.clone(),
                        owner,
    ...
    fn local_success_write(&self, owner: Ident) -> Result<LocalMethodBody, Error> {
        ...
        match FallibleMethodSuccess::new(local_return.plan)? {
            ...
            FallibleMethodSuccess::Closure { closure } => {
                let writer =
                    S::write_closure_return(wrapper::returns::closure::WriteInput::success(
                        closure,
                        self.source.fallible()?.ok_closure(closure.presence())?,
...
trait CallbackMethodSurface: SurfaceLower {
    fn foreign_direct_record_parameter(
        rust_type: Type,
        value: Ident,
        packed: Ident,
    ) -> Result<ForeignMethodParameterTokens, Error>;

    fn outgoing_scalar_option(
        input: wrapper::returns::scalar_option::Input,
    ) -> Result<wrapper::returns::Tokens, Error>;

    fn incoming_scalar_option(
        input: wrapper::returns::scalar_option::IncomingInput,
    ) -> Result<TokenStream, Error>;

    fn outgoing_direct_vector(
        input: wrapper::returns::direct_vec::Input,
    ) -> Result<wrapper::returns::Tokens, Error>;

    fn incoming_direct_vector(
        input: wrapper::returns::direct_vec::IncomingInput,
    ) -> Result<TokenStream, Error>;

    fn write_closure_return<'expansion, 'lowered>(
        input: wrapper::returns::closure::WriteInput<'expansion, 'lowered, Self>,
    ) -> Result<wrapper::returns::closure::WriteTokens, Error>;
...
impl CallbackMethodSurface for Native {
    fn foreign_direct_record_parameter(
        ...
        Ok(ForeignMethodParameterTokens::new(
            ...
            vec![quote! {
                let #packed = <#rust_type as ::boltffi::__private::Passable>::pack(#value);
            }],
            quote! { #packed },
        ))
    }
    ...
    fn handle_carrier(
        ...
        wrapper::handle::CarrierTokens::native(carrier)
    }
    ...
    fn foreign_closure_return_body<'lowered>(
        ...
        let tokens = wrapper::param::closure::Input::returned(
            closure,
            source,
            value.clone(),
            quote! {
...
impl CallbackMethodSurface for Wasm32 {
    fn foreign_direct_record_parameter(
        ...
        let pointer = wrapper::names::Parameter::new(&value).pointer();
        Ok(ForeignMethodParameterTokens::new(
            quote! { *const u8 },
            vec![quote! {
                let #packed = <#rust_type as ::boltffi::__private::Passable>::pack(#value);
    ...
    fn handle_carrier(
        ...
        wrapper::handle::CarrierTokens::wasm32(carrier)
    }
...
impl CallbackEncodedReturn {
    ...
    fn foreign_body<'lowered, S: SurfaceLower>(
        &self,
        call: TokenStream,
        codec: &CodecNode,
        rust_type: Type,
    ...
    fn local_proxy_body<'lowered, S: SurfaceLower>(
        &self,
        call: TokenStream,
        codec: &CodecNode,
        rust_type: Type,
    ...
    fn local_body<'lowered, S: SurfaceLower>(
        &self,
        call: TokenStream,
        codec: &CodecNode,
        expansion: &Expansion<'lowered, S>,
...
struct PackedEncodedValue<'expansion, 'lowered, S: SurfaceLower> {
    packed: TokenStream,
    codec: &'lowered CodecNode,
    rust_type: Type,
    source: &'lowered TypeExpr,
...
impl<'expansion, 'lowered, S: SurfaceLower> PackedEncodedValue<'expansion, 'lowered, S> {
    fn new(
        packed: TokenStream,
        codec: &'lowered CodecNode,
        rust_type: Type,
