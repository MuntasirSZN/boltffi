...
pub struct Export<'expansion, 'lowered, S: boltffi_binding::SurfaceLower> {
...
impl<'expansion, 'lowered> Export<'expansion, 'lowered, boltffi_binding::Native> {
    pub fn new(
        symbol: &'lowered NativeSymbol,
        callable: &'lowered ExportedCallable<boltffi_binding::Native>,
        source: rust_api::Callable<'lowered>,
        rust_call: RustCall,
        receiver: ReceiverTokens,
        visibility: TokenStream,
    ...
    pub fn render(self) -> Result<TokenStream, Error> {
        ...
        let return_tokens = wrapper::returns::Input::new(
        ...
        Ok(quote! {
            ...
            #visibility #safety extern "C" fn #export_ident(#(#ffi_parameters),*) #return_type {
                ...
                // callback waits can fail fast instead of deadlocking.
                #[allow(unexpected_cfgs)]
                #[cfg(boltffi_dart)]
                let _boltffi_dart_sync_ffi = ::boltffi::__dart_runtime::SyncFfiScope::enter();
                #body
            }
        })
    }
    ...
    fn failure(&self) -> Result<TokenStream, Error> {
        match self
            ...
            .params()
            .iter()
            .any(wrapper::param::native_requires_failure_return)
            || self.receiver.requires_failure_return()
        {
            true => wrapper::returns::FailureInput::new(
                self.callable.returns(),
                self.callable.error(),
                self.source.returns(),
                self.expansion,
...
impl<'expansion, 'lowered> Export<'expansion, 'lowered, boltffi_binding::Wasm32> {
    pub fn new(
        symbol: &'lowered NativeSymbol,
        callable: &'lowered ExportedCallable<boltffi_binding::Wasm32>,
        source: rust_api::Callable<'lowered>,
        rust_call: RustCall,
        receiver: ReceiverTokens,
        visibility: TokenStream,
    ...
    fn failure(&self) -> Result<TokenStream, Error> {
        match self
            ...
            .params()
            .iter()
            .any(wrapper::param::wasm32_requires_failure_return)
            || self.receiver.requires_failure_return()
        {
            true => wrapper::returns::FailureInput::new(
                self.callable.returns(),
...
impl RustCall {
    ...
    pub fn associated_constant(owner: TokenStream, constant: syn::Ident) -> Self {
        Self {
            owner: constant.clone(),
            target: RustCallTarget::AssociatedConstant { owner, constant },
        }
    }
    ...
    pub fn expression(&self, arguments: &[TokenStream]) -> TokenStream {
        match &self.target {
            ...
            RustCallTarget::AssociatedConstant { owner, constant } => {
                debug_assert!(arguments.is_empty());
