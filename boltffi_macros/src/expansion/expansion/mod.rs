...
mod tests {
    ...
    use super::Expansion;
    use crate::expansion::{error::Error, wrapper};
    ...
    trait ExpansionTarget: boltffi_binding::SurfaceLower {
        fn function<'lowered>(
    ...
    impl ExpansionTarget for Native {
        fn function<'lowered>(
            expansion: &Expansion<'lowered, Self>,
            source: &'lowered FunctionDef,
        ) -> Result<TokenStream, Error> {
            wrapper::function::Function::new(expansion.function(source)?, expansion).render()
        }
        ...
        fn constant<'lowered>(
            ...
            wrapper::constant::Constant::new(expansion.constant(source)?, expansion).render()
        }
    }

    impl ExpansionTarget for Wasm32 {
        fn function<'lowered>(
            expansion: &Expansion<'lowered, Self>,
            source: &'lowered FunctionDef,
        ) -> Result<TokenStream, Error> {
    ...
    where
        S: ExpansionTarget,
    {
        let wrapper = S::function(expansion, source)?;
    ...
    where
        S: ExpansionTarget,
    {
        S::enumeration(expansion, source)
    }
    ...
    where
        S: ExpansionTarget,
    {
        S::stream(expansion, stream, owner)
    }
    ...
    fn function_wrappers_compose_native_and_wasm_without_reemitting_source_item() {
        ...
        let native_wrapper = wrapper::function::Function::new(
            native_expansion.function(&source.functions[0]).unwrap(),
            &native_expansion,
        )
        .render()
        .expect("native wrapper");
        let wasm_wrapper = wrapper::function::Function::new(
